...
const renderInputButton = (isSelected, style, displayOnOffLabel, props) => {
  return (
    <div {...((props?.hasEdit)&&({className:`${isSelected?'has-edits-switch-selected':'has-edits-switch'}`}))} style={{...style, ...{ backgroundColor: style.fill }, ...STYLES.background}}>
  ...
  return (
    <RadioCheckboxBase
      ...
      renderInputButton={(isSelected, style) =>
        renderInputButton(isSelected, style, props.displayOnOffLabel, props)
      }
      {...props}
    />
  )
