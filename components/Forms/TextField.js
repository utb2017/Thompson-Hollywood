...
class TextField extends React.Component {
  ...
  render() {
    ...
    const inputId = id
    const showHintText =
      hintText && !hasValue && (isFocused || Boolean(type === "date"))
      const showRateText =
        rate && (isFocused || Boolean(type === "date") || hasValue)
        ...
        <div style={styles.inputContainer}>
          {floatingLabelText && (
            <FloatingLabel
              ...
              float={isFocused || hasValue || Boolean(type === "date")}
              disabled={disabled}
              isActive={isFocused}
              hasError={hasError}
              htmlFor={inputId}
          ...
          <input
            {...((hasEdit&&!isFocused)&&{className:'has-edits'})}
            value={value}
            id={inputId}
            ref={(node) => {
              this.input = node
