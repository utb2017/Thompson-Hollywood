...
import { isEmpty, isStr } from "../../helpers";
import firebase, {
  updateFirestore,
  getUserByPhone,
  deleteAuthUser,
...
const Promo = () => {
  ...
  const defaultForm = new PromoClass(
    fireUser?.data?.promo, // promo:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as PromoClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const createUser = useCallback(async () => {
    ...
    if (isEmpty(_form?.promo)) {
      _error.promo = "Name required."
  ...
  const handleClickX = useCallback(async () => {
    if (form?.promo?.length) {
      setLoading(true)
      try {
        const fieldUpdate = { promo: form?.promo }
        await updateFirestore("users", fireUser.data.uid, fieldUpdate)
        //NotificationManager.success("Promo Updated")
      dequeue();
      enqueue({ message: "Promo Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Name not Updated` } }));
        dequeue();
        //NotificationManager.error(e.message)
      }finally{ 
       // setLoading(false)
  ...
  const handleClick = useCallback(async () => {
    const code = isStr(form.promo)
    if (code?.length) {
      setLoading(true);
      enqueue({ message: "Applying Promo", progress: true }, DURATION.infinite);
      try {
        ...
        if(data?.discountAdded === true){
         // NotificationManager.success("Promo applied.")
      dequeue();
      enqueue({ message: "Promo Added", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
          ...
          //setError(`${e?.message || e}`);
          setError((oldError: Errors) => ({ ...oldError, ...{ server: `Promo not applied` } }));
          dequeue();
          showToast(`${error?.message || error}`);
          enqueue({ message: `${error?.message || error}`, startEnhancer: ({ size }) => <DeleteAlt size={size} /> }, DURATION.short);
      } finally {
        setLoading(false);
      }
  ...
  }, [form?.promo, customerID])



  return (
    <>
      <ModalHeader>Enter your promo code.</ModalHeader>
      <ModalBody>
        {/** Title */}
        {/* <FormSection ref={titleRef}>
          <FormInput style={formStyle} label={<Label2>{"User Name"}</Label2>} stack={true}> */}
            <FormControl error={error?.promo}>
              <Input
                required
                disabled={loading}
                onChange={(e) => {
                  const str = e?.currentTarget?.value;
                  setForm((oldForm: PromoClass) => ({ ...oldForm, ...{ promo: str } }));
                }}
                value={form?.promo}
                onFocus={() => (
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireUser?.data?.promo === form?.promo || !form?.promo?.length } isLoading={loading} onClick={handleClick}>
          Apply
        </ModalButton>
      </ModalFooter>
    </>
