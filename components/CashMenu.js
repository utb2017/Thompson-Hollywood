...
function CashMenu() {
  ...
  /** Order Sum Text Ref **/
  useEffect(() => {
    let text = '0 Orders Completed'
    if (completeOrders === 1) {
      text = '1 Order Completed'
    } else if (completeOrders > 1) {
      text = `${orders.length} Orders Completed`
    }
    orderSumRef.current.innerHTML = text
  }, [completeOrders])
  ...
  useEffect(() => {
    let text = '$0.00'
    text = cashOwed
    cashOwedRef.current.innerHTML = text
  }, [cashOwed])
    ...
    <>
      ...
      <ul ref={menuRef} className='side-menu right'>
        ...
        <li className='side-menu-item side-menu-header'>
          <div>
            <h2>{timeSpan}</h2>
            <p ref={orderSumRef} className='menu-item-green'>
              0 orders completed.
            </p>
          </div>
        </li>
