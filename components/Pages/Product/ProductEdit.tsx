import { useState, useEffect, useRef, Fragment, ReactElement, useCallback } from "react";
...
const FlexBox = styled("div", ({ $theme, $flex = 1 }) => {
  return {
    flex: $flex,
    width: `100%`,
  };
...
const EditProduct = ({ fireProduct }): ReactElement => {
  const {
    lastID,
    setLastID,
    firstID,
  ...
  //const [fireProductState, setFireProductState] = useState<ProductClass>(fireProduct);
  const [fireProductDefault, setFireProductDefault] = useState<ProductClass>({ ...fireProduct });

  const [value, setValue] = useState([]);
  useEffect(() => {
    //alert(JSON.stringify(fireStoreQuery))
    const thisProductLive = (fireStoreQuery?.data || []).filter((x: ProductClass) => x.id === fireProduct.id);
    //alert('update default' )

    setFireProductDefault({ ...defaultForm, ...thisProductLive[0] });
  }, [fireStoreQuery.data]);

  //alert(JSON.stringify(fireProduct))
  ...
  useEffect(() => {
    ...
    //let changes:boolean = !deepEqual(fireProductDefault, form)
    let changes: boolean = false;
    const _default: ProductClass = { ...fireProductDefault };
    const _state: ProductClass = { ...form };
    ...
    // if(!deepEqual(fireProductDefault, form)){
    //   changes = true
    // }
  ...
  useEffect(() => {
    ...
    const thisProductLive = (fireStoreQuery?.data || []).filter((x: ProductClass) => x.id === fireProduct.id);
    setForm({ ...defaultForm, ...thisProductLive[0] } as ProductClass);
    console.log(JSON.stringify({ ...thisProductLive[0] }))
    return () => {
      setForm({});
      setError({});
    };
  ...
  const updateProduct = async () => {
    ...
    //alert(JSON.stringify(form?.brand))
    //return

    setLoading(true);
    enqueue({ message: "Updating Product", progress: true }, DURATION.infinite);
    try {
      const updateProduct = firebase.functions().httpsCallable("updateProduct");
      const response = await updateProduct(_form);
      dequeue();
      enqueue({ message: "Product Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      ...
      setError((oldError: Errors) => ({ ...oldError, ...{ server: `Product not updated` } }));
      dequeue();
      showToast(`${e?.message || e}`);
      enqueue({ message: `Your product wasn't updated`, startEnhancer: ({ size }) => <DeleteAlt size={size} /> }, DURATION.short);
      closeModal()
    } finally {
      setLoading(false);
    }
  ...
  const getImgURL = async (filePath:string) => {
  ...
  useEffect(() => {
    //let imgPath = `placeholders/placeholder-images-image_license.png`
  //  alert(form?.filePath)
    if(typeof form?.filePath === 'string'){
      //alert('set file path')
      //imgPath = form.filePath
      getImgURL(form?.filePath)
    }
    //getImgURL(imgPath)
  }, [form])
    ...
    <>
      ...
      <ModalBody>
        ...
        <FormSection ref={collectionRef}>
          <FormInput style={formStyle} label={<Label2>{"Collection"}</Label2>} stack={true}>
            <FormControl error={error?.collections}>
              <Select
                ...
                type={TYPE.search}
                multi
                onChange={({value}) => setForm((oldForm: ProductClass) => ({ ...oldForm, collections: value }))}
                ...
                //   }
                //   setForm((oldForm: ProductClass) => ({ ...oldForm, collections: filteredSelected, collectionIDs: filteredString }));
          ...
          <FormInput style={formStyle} label={<Label2>{"Brand"}</Label2>} stack={true}>
            <FormControl error={error?.brand}>
              {<Select
                ...
                //multi
                clearable={false}
                value={form?.brand?Array.isArray(form?.brand)?form?.brand:[form.brand]:undefined}
                labelKey="label"
                valueKey="value"
        ...
        <FormSection ref={imageRef}>
          <FormInput style={formStyle} label={<Label2>{"Image"}</Label2>} stack={true}>
          {
              ...
              overrides={{
                ...
                headerImage={
                  (photoURL) ? photoURL :(imgURL)?imgURL:<Skeleton  height="400px" width="100%"  />
                }
            ...
            <FlexContainer>
              <FormControl caption={() => 'Type'} error={error?.type}>
                <Select
                  ...
                  //multi
                  //clearable={false}
                  value={form?.genome ? Array.isArray(form?.genome)?form?.genome:[form.genome]:undefined  }
                  labelKey="label"
                  valueKey="value"
        ...
        <Accordion>
          ...
          <Panel title="Database Dev">
            {
              <>
                {fireProductDefault &&
                  Object.keys(fireProductDefault).map(function (key, index) {
                    //alert(fireProductDefault[key])
          ...
          <Panel title="Change Error">{<>{JSON.stringify(changeError)}</>}</Panel>
        </Accordion>
        <>
          <ToasterContainer
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={!hasChanges} isLoading={loading} onClick={updateProduct}>
          Update
        </ModalButton>
      </ModalFooter>
    </>
