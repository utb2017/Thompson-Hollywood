...
import { capitalize, isEmpty, isNum } from "../../helpers"
...
const Orders = () => {
  ...
  const fireCollectionsTotal = useFirestoreQuery(
    user?.uid && query && firebase.firestore().collection("totals").doc('unsettled')
  )
  const collectionTotal = isNum(fireCollectionsTotal.data?.[router.query.filter]) 
    ...
    user?.uid &&
      setRef(
        firebase
          ...
          .where("progress", "in", orderProgressObject[`${router?.query?.filter || ""}`])
          .where("settled", "==", false)
      )
