...
export default function OrdersTable({form}) {
  ...
  useEffect(() => {
    ...
    // alert(JSON.stringify(fireStoreQuery))
     console.log(JSON.stringify(fireStoreQuery))
    // alert(JSON.stringify(queryLoader))
    if (dataList.length && !queryLoader) {
      setDataState(dataList)
    } else if (!dataList.length && !queryLoader) {
      setDataState([])
    }
  }, [dataList, queryLoader, fireStoreQuery])
    ...
    <Card
      ...
      <CustomTable style={{position: 'relative'}}>
        <StyledHead $width='100%'>
          ...
          {!isMobile && <StyledHeadCell style={{flex: 1}}>Total</StyledHeadCell>}
          {!isMobile && <StyledHeadCell style={{flex: 1}}>Items</StyledHeadCell>}
          {<StyledHeadCell style={{flex: 1}}>Progress</StyledHeadCell>}
            ...
            dataState.map((row: OrderClass, index: number) => (
              <StyledRow key={index}>
                ...
                <StyledBorderCell
                  onClick={() => {
                    router.push(
                      `/[adminID]/orders/selected/[uid]/[oid]`,
                      `/${user.uid}/orders/selected/${row.user}/${row.id}`
                    )
                  }}
                  style={{flex: 2}}>
                ...
                {!isMobile && (
                  <StyledBorderCell
                    ...
                    style={{flex: 1}}>
                    {row?.cartTotals?.grandTotal && <CellWrapper>{isCurr(row.cartTotals.grandTotal)}</CellWrapper>}
                  </StyledBorderCell>
                )}
                {!isMobile && (
                  <StyledBorderCell
                ...
                )}
                {
                  <StyledBorderCell
                    ...
                    <CellWrapper>
                      <Tag
                        //disabled={true}
                        kind={row.progress === 'received' ? _KIND.red : undefined}
                        overrides={{
                          Root: {
                            style: ({$theme}) => ({
                              marginBottom: '0px',
