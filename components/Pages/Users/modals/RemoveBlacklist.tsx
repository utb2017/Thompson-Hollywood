...
const RemoveBlacklist = ({id}) => {
  ...
  //   setNavLoading(true)
  //   try{
  //     deleteBlacklisting({discountID:id, userID:fireCustomer.data.id})
  ...
  const handleClick = async (id:string) => {
    if (form?.role?.length) {
      setLoading(true)
      try {
        await  deleteBlacklisting({discountID:id, userID:fireCustomer.data.id})
      ...
      closeModal()
        ...
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Discount not deleted` } }));
        dequeue();
  ...
  return (
    <>
      <ModalHeader>Remove Blacklist Discount</ModalHeader>
      <ModalBody>
        This action cannot be undone. The user will be able to use this discount again.
        {/* {`${id}`} */}
        <>
          <ToasterContainer
