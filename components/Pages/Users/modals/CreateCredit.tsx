...
const DisplayName = () => {
  ...
  const defaultForm = new DisplayNameClass(
    fireCustomer?.data?.displayName, // displayName:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as DisplayNameClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = async () => {
    ...
    if (!(form?.initialAmount &&form?.initialAmount?.length > 0)) {
      return alert("no total")  
    }
    if (!(fireCustomer?.data?.uid && fireCustomer?.data?.uid.length > 0)) {
      return alert("no id")
    }
    ...
    //   const response = await call({user:fireCustomer?.data?.uid, amount:form?.inititalAmount, title:form?.title})
    ...
    setLoading(true);
    enqueue({ message: "Creating Credit", progress: true }, DURATION.infinite);
    try {
      const call = firebase.functions().httpsCallable("addCredit");
      const response = await call({user:fireCustomer?.data?.uid, amount:form?.initialAmount, title:form?.title})
      dequeue();
      enqueue({ message: "Credit Created", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
    } catch (error:any) {
        //setError(`${e?.message || e}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Credit not created` } }));
        dequeue();
        showToast(`${error?.message || error}`);
        enqueue({ message: `Credit wasn't added`, startEnhancer: ({ size }) => <DeleteAlt size={size} /> }, DURATION.short);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <ModalBody>
        ...
        <FormSection ref={titleRef}>
          {/* <FormInput style={formStyle} label={<Label2>{"Title"}</Label2>} stack={true}> */}
            <FormControl error={error?.title}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: DisplayNameClass) => ({ ...oldForm, ...{ title: str } }));
                }}
                value={form?.title}
                onFocus={() => (
            ...
            <FormControl error={error?.initialAmount}>
              <Input
                required
                disabled={loading}
                onChange={(e) => {
                  const str = e?.currentTarget?.value;
                  setForm((oldForm: DisplayNameClass) => ({ ...oldForm, ...{ initialAmount: str } }));
                }}
                ...
                id="initialAmount"
                name="initialAmount"
                error={Boolean(error?.initialAmount)}
                type="number"
                placeholder="20"
                //clearable
                clearOnEscape
        ...
        {/** Amount */}
        {/* <FormSection ref={titleRef}>
          <FormInput style={formStyle} label={<Label2>{"Amount"}</Label2>} stack={true}> */}
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireCustomer?.data?.initialAmount === form?.initialAmount || !form?.initialAmount?.length } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
