...
const Users: FC = (): ReactElement => {
  ...
  useEffect(() => {
      ...
      setTotalsField(`${router?.query?.role}`);
      setTotalsDoc("users");
      setTotalsCollection("totals")


      setWhere([ [ router.query?.role === "pending"?"status":"role", "==", `${router?.query?.role}` ] ])
      setQueryCollection("users");
      setOrderBy("id");
  ...
  const _userCreate = () => {
    const component: () => ReactElement = () => <UserCreate />;
