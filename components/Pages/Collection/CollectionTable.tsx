...
import { defaultOrders, isCurr } from "../../../helpers";
...
export default function CollectionTable({ sortFilter }) {
  const [css, theme] = useStyletron();
  const [dataState, setDataState] = useState<CollectionClass[] | []>([]);
  ...
  const saveOrder = React.useCallback( async () => {
    if (isEqual(defaultOrder, searchCollections)) {

      //return alert('no changes')
    } else {
      //key=key.replace(/\s+/g,"_")
      const newOrderIDstr = []
      for (const orderNameStr of searchCollections) {
        //alert(JSON.stringify(orderNameStr))
        for (const key in fireCollections.data) {
          const original = fireCollections.data[key]
      ...
      if (Array.isArray(newOrderIDstr) && newOrderIDstr.length) {
        ...
        //setLoading(true);
        enqueue({ message: "Updating collections sort", progress: true }, DURATION.infinite);
        try {
          const updateCollectionOrder = firebase.functions().httpsCallable("updateCollectionOrder");
          const response = await updateCollectionOrder(newOrderIDstr);
          dequeue();
          enqueue({ message: "Collections updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
          ...
          enqueue({ message: `Your collection wasn't updated`, startEnhancer: ({ size }) => <DeleteAlt size={size} /> }, DURATION.short);
        } finally {
          //setLoading(false);
        }
  ...
  }, [defaultOrder, searchCollections, fireCollections])
  ...
  useEffect(() => {
    ...
    if (fireCollections.data) {
      ...
      for (const key in data) {
        ...
        tempCollectionList.push(`${collection?.title}`)
    ...
    setDefaultOrder(tempCollectionList)
  ...
  useEffect(() => {
    if (dataList.length) {
      const x = dataList as CollectionClass[]
      setDataState(x);
    } else {
      if (!queryLoader) {
        setDataState([]);
  ...
  const _editCollection = (fireCollection: CollectionClass) => {
    const component = () => <CollectionEdit fireCollection={fireCollection} />;
    openModalBase(component, true);
  };

  const _deleteCollection = (fireCollection: CollectionClass) => {
    const component: () => ReactElement = () => <CollectionDelete fireCollection={fireCollection} />;
      ...
      <CustomTable>
        ...
        <StyledBody $width="100%" style={{ minHeight: `${limit * (width > 450 ? 79 : 79)}px` }}>
          ...
          {dataState && !sortFilter &&
            dataState.map((row: CollectionClass, index: number) => (
              <StyledRow style={{ zIndex: 3 }} key={index}>
                <StyledBorderCell onClick={() => _editCollection(row)} style={{ minWidth: "48px", flex: 0 }}>
                  <CellWrapper>
                    {row.active?<Show />:<Hide/> }
                  </CellWrapper>
                </StyledBorderCell>
                <StyledBorderCell onClick={() => _editCollection(row)} style={{ flex: 2 }}>
                  <CellWrapper onClick={() => _editCollection(row)}>{row.title}</CellWrapper>
                </StyledBorderCell>
                {/* {!isMobile && (
                  <StyledBorderCell style={{ flex: 2, paddingTop: `0px`, paddingBottom: "0px" }} onClick={() => _editCollection(row)}>
                ...
                {!isMobile && (
                  <StyledBorderCell onClick={() => _editCollection(row)} style={{ flex: 1 }}>
                ...
                <StyledBorderCell style={{ minWidth: "80px", flex: 0 }}>
                  <StyledAction style={{ width: "100%" }} onClick={() => _deleteCollection(row)}>
          ...
          {sortFilter && <List
            ...
            onChange={({ oldIndex, newIndex }) =>
              setSearchCollections(
                newIndex === -1
                  ? arrayRemove(searchCollections, oldIndex)
                  : arrayMove(searchCollections, oldIndex, newIndex)
              )
              // setItems(
              //   newIndex === -1
