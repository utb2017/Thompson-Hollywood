        ...
        <StyledBody $width="100%" style={{ minHeight: `${limit * (width > 450 ? 79 : 73)}px` }}>
          ...
          {dataState &&
            dataState.map((row: DiscountClass, index: number) => (
              <StyledRow key={index}>
                <StyledBorderCell onClick={() => _editDiscount(row)} style={{ minWidth: "40px", flex: 0 }}>
                  <CellWrapper>
                    <Indicator $isActive={row?.active} $featured={row?.featured} />
                  </CellWrapper>
                </StyledBorderCell>
                <StyledBorderCell onClick={() => _editDiscount(row)} style={{ flex: 2 }}>
                  <CellWrapper onClick={() => _editDiscount(row)}>{row?.title}</CellWrapper>
                </StyledBorderCell>
                {!isMobile && (
                  <StyledBorderCell style={{ flex: 2, paddingTop: `0px`, paddingBottom: "0px" }} onClick={() => _editDiscount(row)}>
                ...
                {!isMobile && (
                  <StyledBorderCell onClick={() => _editDiscount(row)} style={{ flex: 1 }}>
                ...
                <StyledBorderCell style={{ minWidth: "80px", flex: 0 }}>
                  <StyledAction style={{ width: "100%" }} onClick={() => _deleteDiscount(row)}>
