...
const defaultFormX = new DiscountClass(
  true, // active: boolean,
  false, // alert: boolean,
  2, // bogoQty: number,
  `HH`, // code: string,
  ...
  null, // id: string,
  { label: "Percent", value: "percent" }, // method: Selected,
  "percent", // methodID: "flatRate" | "percent" | "taxFree" | "bogo",
  10, // rate: number,
  true, // recurring: boolean,
  [{value: '2', label: 'Tuesdays'},{value: '3', label: 'Wednesdays'}], // recurringDays: Selected[] | undefined,
  //"coupon", // sort: "coupon",
  false, // stackable: boolean,
  `Happy Hour`, // title: string | null,
  //{ label: "Coupon", value: "coupon" }, // type: { [k: string]: any } | undefined,
  null, // uid: string | null,
  false, // used: boolean,
  false, //happyhour
  { label: new Date(start.toUTCString()), value: 17 }, /// start hour
  { label: new Date(end.toUTCString()), value: 18 } //endHour
);
...
const CreateDiscount = () => {  const {
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as DiscountClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  useEffect(() => {
    //alert(JSON.stringify(form?.collections))
    //alert('lection change')
    if (Array.isArray(form?.collections)) {
      if (form?.collections.length === 0) {
        setForm((oldForm: DiscountClass) => ({
          ...oldForm,
          ...{ collectionIDs: ["ALL_PRODUCTS"], collections: [{ label: "Cart Total", value: "ALL_PRODUCTS" }] },
        }));
      }
  ...
  /* reset errors */
  useEffect(() => {
    //const hasAll = ((form?.collections?.filter((item:Selected) => item.value === 'ALL_PRODUCTS'))||[]).length
    //alert(hasAll)
    closeToast();
    setError({});
    return () => {
  ...
  const createDiscount = async () => {
    ...
    setLoading(true);
    enqueue({ message: "Creating Discount", progress: true }, DURATION.infinite);
    try {
      const createDiscount = firebase.functions().httpsCallable("createDiscount");
      const response = await createDiscount(_form);
      dequeue();
      enqueue({ message: "Discount Created", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      //if (response?.data?.success === true) {}
    } catch (e) {
      //setError(`${e?.message || e}`);
      setError((oldError: Errors) => ({ ...oldError, ...{ server: `Discount not created` } }));
      dequeue();
      showToast(`${e?.message || e}`);
      enqueue({ message: `Your discount wasn't added`, startEnhancer: ({ size }) => <DeleteAlt size={size} /> }, DURATION.short);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <ModalBody>
                ...
                <div className="dual-input">
                  <FormControl error={error?.startHour}>
                    <TimePicker
                      ...
                      //value={form?.startHour?.label}
                      error={Boolean(error?.startHour)}
                      onChange={(date) =>
                        setForm((oldForm: DiscountClass) => ({ ...oldForm, startHour: { label: date, value: date.getHours() } }))
                      }
                      overrides={{
                        Select: {
                          props: {
...

        {
          <>
           {/*   <div>Form</div>
             {
                // Object.entries(fireDiscountDefault).map(function(key, value) {
                //     <>{`${key} : ${value}`}</>
                // })
                form &&
                  Object.keys(form).map(function (key, index) {
        ...
        {/*    <div style={{ width: "100%", height: 65 }}></div>

              {
                  <>
                    ...
                    //     <>{`${key} : ${value}`}</>
                    // })  
                    fireDiscountDefault && Object.keys(fireDiscountDefault).map(function(key, index) {
                      //alert(fireDiscountDefault[key])
      ...
      <ModalFooter>
        ...
        <ModalButton isLoading={loading} onClick={createDiscount}>
          Create
        </ModalButton>
      </ModalFooter>
    </>
