...
const License = () => {
  ...
  const defaultForm = new LicenseClass(
    fireUser?.data?.photoURL, // photoURL:string,
  );
  ...
  //     Boolean(fireUser?.data?.uid) && updateProfile(photoURL)
  //     closeModal()
  //     return
  //   } catch (e) {
  //     setError(`${e?.message||e||'ERROR'}`)
  ...
  return (
    <>
          ...
          <FormInput style={formStyle} label={<Label2>{"License"}</Label2>} stack={true}> */}
            {
              ...
              overrides={{
                ...
                HeaderImage: {
                  style: ({ $theme }) => ({ width:'100%', maxWidth:'420px', minHeight:'100%', height:'auto', minWidth:'265px' })
                }
              }}
                headerImage={
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireUser?.data?.photoURL === form?.photoURL || !form?.photoURL?.length } 
        isLoading={loading} 
        onClick={()=>{
