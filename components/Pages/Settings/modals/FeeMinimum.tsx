...
class FreeDeliveryMinimumClass {
  freeDeliveryMin:string;
  constructor(
    freeDeliveryMin:string,
  ) {
    this.freeDeliveryMin = freeDeliveryMin;
  }
}
interface Errors {
  title?: string;
...
const FreeDeliveryMinimum = () => {
  ...
  const defaultForm = new FreeDeliveryMinimumClass(
    fireSettings?.data?.freeDeliveryMin, // freeDeliveryMin:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as FreeDeliveryMinimumClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = async () => {
    ...
    if (!(form?.freeDeliveryMin && form?.freeDeliveryMin?.length && typeof form?.freeDeliveryMin === 'number')) {
      //_error.freeDeliveryMin = "Delivery fee required."
      //return setError({ ..._error })
    }
      setLoading(true)
      try {
        const fieldUpdate = { freeDeliveryMin: form?.freeDeliveryMin }
        await updateFirestore("admin", 'store', fieldUpdate)
        //NotificationManager.success("Free Delivery - Minimum Updated")
      dequeue();
      enqueue({ message: "Free Delivery - Minimum Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Free Delivery - Minimum not Updated` } }));
        dequeue();
  ...
  return (
    <>
            ...
            <FormControl error={error?.freeDeliveryMin}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: FreeDeliveryMinimumClass) => ({ ...oldForm, ...{ freeDeliveryMin: str } }));
                }}
                ...
                error={Boolean(error?.freeDeliveryMin)}
                type="number"
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={form?.freeDeliveryMin === fireSettings?.data?.freeDeliveryMin } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
...
export default FreeDeliveryMinimum;
