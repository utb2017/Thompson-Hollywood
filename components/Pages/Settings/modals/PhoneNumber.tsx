...
const PhoneNumber = () => {
  ...
  const defaultForm = new PhoneNumberClass(
    fireUser?.data?.phoneNumber, // phoneNumber:string,
  );
  ...
  const handleClick = useCallback(async () => {
    if (form?.phoneNumber?.length) {
      setLoading(true)
      try {
        const fieldUpdate = { phoneNumber: form?.phoneNumber }
        await updateFirestore("users", fireUser.data.uid, fieldUpdate)
        //NotificationManager.success("Phone Number Updated")
        dequeue();
        enqueue({ message: "Phone Number Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
        closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Phone not Updated` } }));
        dequeue();
        //NotificationManager.error(e.message)
      } finally {
        // setLoading(false)
  ...
  return (
    <>
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireUser?.data?.phoneNumber === form?.phoneNumber || !form?.phoneNumber?.length} isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
