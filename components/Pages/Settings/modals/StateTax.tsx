...
const StateTax = () => {
  ...
  const defaultForm = new StateTaxClass(
    fireSettings?.data?.stateTax, // stateTax:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as StateTaxClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = async () => {
    ...
    if (!(form?.stateTax && form?.stateTax?.length && typeof form?.stateTax === 'number')) {
      //_error.stateTax = "Delivery fee required."
      //return setError({ ..._error })
    }
      setLoading(true)
      try {
        const fieldUpdate = { stateTax: form?.stateTax }
        await updateFirestore("admin", 'store', fieldUpdate)
        //NotificationManager.success("StateTax Updated")
      dequeue();
      enqueue({ message: "StateTax Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `StateTax not Updated` } }));
        dequeue();
  ...
  return (
    <>
            ...
            <FormControl error={error?.stateTax}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: StateTaxClass) => ({ ...oldForm, ...{ stateTax: str } }));
                }}
                value={roundTo((form?.stateTax*100),2)||''}
                onFocus={() => (
                ...
                id="stateTax"
                name="stateTax"
                error={Boolean(error?.stateTax)}
                type="number"
                placeholder="StateTax"
                //clearable
                //clearOnEscape
                overrides={{
                  Root: {
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={form?.stateTax === fireSettings?.data?.stateTax } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
