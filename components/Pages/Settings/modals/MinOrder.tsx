...
const MinOrder = () => {
  ...
  const defaultForm = new MinOrderClass(
    fireSettings?.data?.minOrder, // minOrder:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as MinOrderClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = async () => {
    ...
    if (!(form?.minOrder && form?.minOrder?.length && typeof form?.minOrder === 'number')) {
      //_error.minOrder = "Delivery fee required."
      //return setError({ ..._error })
    }
      setLoading(true)
      try {
        const fieldUpdate = { minOrder: form?.minOrder }
        await updateFirestore("admin", 'store', fieldUpdate)
        //NotificationManager.success("Minimum Order Updated")
      dequeue();
      enqueue({ message: "Minimum Order Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Minimum Order not Updated` } }));
        dequeue();
  ...
  return (
    <>
            ...
            <FormControl error={error?.minOrder}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: MinOrderClass) => ({ ...oldForm, ...{ minOrder: str } }));
                }}
                value={form?.minOrder}
                onFocus={() => (
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={form?.minOrder === fireSettings?.data?.minOrder } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
