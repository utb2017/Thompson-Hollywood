...
const ExciseTax = () => {
  ...
  const defaultForm = new ExciseTaxClass(
    fireSettings?.data?.exciseTax, // exciseTax:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as ExciseTaxClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = async () => {
    ...
    if (!(form?.exciseTax && form?.exciseTax?.length && typeof form?.exciseTax === 'number')) {
      //_error.exciseTax = "Delivery fee required."
      //return setError({ ..._error })
    }
      setLoading(true)
      try {
        const fieldUpdate = { exciseTax: form?.exciseTax }
        await updateFirestore("admin", 'store', fieldUpdate)
        //NotificationManager.success("Excise Tax Updated")
      dequeue();
      enqueue({ message: "Excise Tax Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Excise Tax not Updated` } }));
        dequeue();
  ...
  return (
    <>
          ...
          <FormInput style={formStyle} label={<Label2>{"User Name"}</Label2>} stack={true}> */}
            <FormControl error={error?.exciseTax}>
              <Input
                ...
                endEnhancer={'%'}
                onChange={(e) => {
                  let str:string|number = e?.currentTarget?.value;
                  str = (Number(str)/100)
                  //str = (Math.round(str * 100) / 100)
                  setForm((oldForm: ExciseTaxClass) => ({ ...oldForm, ...{ exciseTax: str } }));
                }}
                value={roundTo((form?.exciseTax*100),2)||''}
                onFocus={() => (
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={form?.exciseTax === fireSettings?.data?.exciseTax } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
