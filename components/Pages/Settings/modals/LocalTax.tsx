...
const LocalTax = () => {
  ...
  const defaultForm = new LocalTaxClass(
    fireSettings?.data?.localTax, // localTax:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as LocalTaxClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = async () => {
    ...
    if (!(form?.localTax && form?.localTax?.length && typeof form?.localTax === 'number')) {
      //_error.localTax = "Delivery fee required."
      //return setError({ ..._error })
    }
      setLoading(true)
      try {
        const fieldUpdate = { localTax: form?.localTax }
        await updateFirestore("admin", 'store', fieldUpdate)
        //NotificationManager.success("LocalTax Updated")
      dequeue();
      enqueue({ message: "Local Tax Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Local Tax not Updated` } }));
        dequeue();
  ...
  return (
    <>
            ...
            <FormControl error={error?.localTax}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: LocalTaxClass) => ({ ...oldForm, ...{ localTax: str } }));
                }}
                value={roundTo((form?.localTax*100),2)||''}
                onFocus={() => (
                ...
                id="localTax"
                name="localTax"
                error={Boolean(error?.localTax)}
                type="number"
                placeholder="LocalTax"
                //clearable
                //clearOnEscape
                overrides={{
                  Root: {
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={form?.localTax === fireSettings?.data?.localTax } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
