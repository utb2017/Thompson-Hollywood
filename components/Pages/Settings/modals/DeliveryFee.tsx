...
const DeliveryFee = () => {
  ...
  const defaultForm = new DeliveryFeeClass(
    fireSettings?.data?.deliveryFee, // deliveryFee:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as DeliveryFeeClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const createUser = useCallback(async () => {
    ...
    if (isEmpty(_form?.deliveryFee)) {
      _error.deliveryFee = "Name required."
  ...
  const handleClick = async () => {
    ...
    if (!(form?.deliveryFee && form?.deliveryFee?.length && typeof form?.deliveryFee === 'number')) {
      //_error.deliveryFee = "Delivery fee required."
      //return setError({ ..._error })
    }
      setLoading(true)
      try {
        const fieldUpdate = { deliveryFee: form?.deliveryFee }
        await updateFirestore("admin", 'store', fieldUpdate)
        //NotificationManager.success("Delivery Fee Updated")
      dequeue();
      enqueue({ message: "Delivery Fee Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Delivery Fee not Updated` } }));
        dequeue();
  ...
  return (
    <>
            ...
            <FormControl error={error?.deliveryFee}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: DeliveryFeeClass) => ({ ...oldForm, ...{ deliveryFee: str } }));
                }}
                ...
                id="deliveryFee"
                name="deliveryFee"
                error={Boolean(error?.deliveryFee)}
                type="text"
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={form?.deliveryFee === fireSettings?.data?.deliveryFee } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
