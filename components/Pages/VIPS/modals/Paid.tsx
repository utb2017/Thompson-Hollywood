...
import { capitalize, isCurr, isEmpty, PROGRESS } from "../../../../helpers";
import firebase, {
  updateFirestore,
  getUserByPhone,
  deleteAuthUser,
...
const ResetOrder = ({order}:{order:OrderClass}) => {
  const {
    lastID,
    setLastID,
    firstID,
  ...
  const handleReset = async (order:OrderClass) => {
    setLoading(true)
    enqueue({ message: "Updating order", progress: true }, DURATION.infinite);
    setTimeout(async () => {
      try {
        ...
        dequeue();
        enqueue({ message: "Order paid", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
        closeModal()
      } catch (e) {
        setError((oldError) => ({ ...oldError, ...{ server: `Error reseting order.` } }));
        dequeue();
        //showToast(`${error?.message || error}`);
        enqueue({ message: `Error paying order`, startEnhancer: ({ size }) => <DeleteAlt size={size} /> }, DURATION.short);
      } finally {
        setLoading(false)
      }
    }, 2000);
  ...
  return (
    <>
      <ModalHeader>Settle Order</ModalHeader>
      <ModalBody>
        {`${order.driverName} has given you ${isCurr(order.cartTotals.grandTotal)}?`}
        {/* <>
          <ToasterContainer
      ...
      <ModalFooter>
        <ModalButton disabled={loading} onClick={closeModal} kind={ButtonKind.tertiary}>No</ModalButton>
        <ModalButton 
        //disabled={fireCustomer?.data?.role === form?.role || !form?.role?.length }
         isLoading={loading}
         disabled={loading}
