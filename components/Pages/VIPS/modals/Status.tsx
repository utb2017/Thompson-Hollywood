...
const Status = () => {
  ...
  const defaultForm = new StatusClass(
    fireCustomer?.data?.status, // status:string,
  );
  ...
  /* form setup */
  useEffect(() => {
    setForm({ ...defaultForm } as StatusClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const createUser = useCallback(async () => {
    ...
    if (isEmpty(_form?.status)) {
      _error.status = "Status required."
  ...
  const handleClick = useCallback(async () => {
    if (form?.status?.length) {
      setLoading(true)
      try {
        const fieldUpdate = { status: form?.status }
        await updateFirestore("users", fireCustomer.data.uid, fieldUpdate)
        //NotificationManager.success("Status Updated")
      dequeue();
      enqueue({ message: "Status Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
  ...
  return (
    <>
          ...
          <FormInput style={formStyle} label={<Label2>{"User Status"}</Label2>} stack={true}> */}
            <FormControl error={error?.status}>
              <Select
                ...
                //   setForm((oldForm: StatusClass) => ({ ...oldForm, ...{ status: str } }));
                // }}
                value={ typeof form?.status === 'string' ? [{label: capitalize(form?.status), value:form?.status}] : Array.isArray(form?.status) ?form?.status :undefined          }
                onFocus={() => (
                ...
                onChange={params => setForm((oldForm: StatusClass) => ({ ...oldForm, ...{ status: params?.value[0].value } }))}
                placeholder="Status"
                clearable={false}
                //clearOnEscape
                overrides={{
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireCustomer?.data?.status === form?.status || !form?.status?.length } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
