...
const Progress = ({order}:{order:OrderClass}) => {
  const {
    lastID,
    setLastID,
    firstID,
  ...
  const { user, fireBrands, fireCollections, fireCustomer, fireProgresss } = useUser();
  ...
  // const defaultForm = new ProgressClass(
  //   order.progress, // progress:string,
  // );
  ...
  /* form setup */
  useEffect(() => {
    //setForm({ ...defaultForm } as ProgressClass);
    return () => {
      setForm({});
      setError({});
    };
  ...
  const handleClick = (order) => {
      ...
      //alert(JSON.stringify(form.progress))
      //return
      setTimeout(async () => {
        try {
          await updateFirestoreGroup(
            ...
            order.id,
            {
              progress: form.progress[0].value,
            }
          );
          dequeue();
  ...
  return (
    <>
      <ModalHeader>Change Progress</ModalHeader>
      <ModalBody>
        {/** Title */}
        {/* <FormSection ref={titleRef}>
          <FormInput style={formStyle} label={<Label2>{"User Progress"}</Label2>} stack={true}> */}
        <FormControl error={error?.progress}>
          <Select
            //required
            disabled={loading}
            // onChange={(e) => {
            //   const str = e?.currentTarget?.value;
            //   setForm((oldForm: ProgressClass) => ({ ...oldForm, ...{ progress: str } }));
            // }}
            value={form?.progress}
            onFocus={() => (
            ...
            onChange={params => setForm((oldForm: ProgressClass) => ({ ...oldForm, ...{ progress: params?.value } }))}
