...
const Role = () => {
  ...
  const defaultForm = new RoleClass(
    fireCustomer?.data?.role, // role:string,
  );
  ...
  const handleClick = useCallback(async () => {
    if (form?.role?.length) {
      setLoading(true)
      try {
        const fieldUpdate = { role: form?.role }
        await updateFirestore("users", fireCustomer.data.uid, fieldUpdate)
        //NotificationManager.success("Role Updated")
      dequeue();
      enqueue({ message: "Role Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        setError((oldError: Errors) => ({ ...oldError, ...{ server: `Role not Updated` } }));
        dequeue();
        //NotificationManager.error(e.message)
      }finally{ 
       // setLoading(false)
    ...
    <>
      ...
      <ModalBody>
        ...
        {/* <FormSection ref={titleRef}>
          <FormInput style={formStyle} label={<Label2>{"User Role"}</Label2>} stack={true}> */}
            <FormControl error={error?.role}>
              <Select
                ...
                //   setForm((oldForm: RoleClass) => ({ ...oldForm, ...{ role: str } }));
                // }}
                value={ typeof form?.role === 'string' ? [{label: capitalize(form?.role), value:form?.role}] : Array.isArray(form?.role) ?form?.role :undefined          }
                onFocus={() => (
                ...
                onChange={params => setForm((oldForm: RoleClass) => ({ ...oldForm, ...{ role: params?.value[0].value } }))}
                placeholder="Role"
                clearable={false}
                //clearOnEscape
                overrides={{
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireCustomer?.data?.role === form?.role || !form?.role?.length } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
