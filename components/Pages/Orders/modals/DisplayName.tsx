...
const DisplayName = () => {
  ...
  const handleClick = useCallback(async () => {
    if (form?.displayName?.length) {
      setLoading(true)
      try {
        const fieldUpdate = { displayName: form?.displayName }
        await updateFirestore("users", fireCustomer.data.uid, fieldUpdate)
        //NotificationManager.success("Display Name Updated")
      dequeue();
      enqueue({ message: "Display Name Updated", startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
      closeModal()
      } catch (e) {
        setLoading(false)
        showToast(`${error?.message || error}`);
        ...
        dequeue();
        enqueue({ message: `Name not Updated`, startEnhancer: ({ size }) => <Check size={size} /> }, DURATION.short);
        //NotificationManager.error(e.message)
      }finally{ 
       // setLoading(false)
      }
  ...
  return (
    <>
            ...
            <FormControl error={error?.displayName}>
              <Input
                ...
                onChange={(e) => {
                  ...
                  setForm((oldForm: DisplayNameClass) => ({ ...oldForm, ...{ displayName: str } }));
                }}
      ...
      <ModalFooter>
        ...
        <ModalButton disabled={fireCustomer?.data?.displayName === form?.displayName || !form?.displayName?.length } isLoading={loading} onClick={handleClick}>
          Save
        </ModalButton>
      </ModalFooter>
    </>
